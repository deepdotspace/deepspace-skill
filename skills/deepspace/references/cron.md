...
Render task status, history, and (optionally) `trigger` / `pause` / `resume` controls with `useCronMonitor(roomId)` from `deepspace`. Pass `SCOPE_ID` from `src/constants.ts` (default `app:${APP_ID}` — the immutable `DEEPSPACE_APP_ID`) to hit the app's `AppCronRoom` DO. The hook returns `{ tasks, history, connected, canWrite, trigger(name), pause(name), resume(name) }`. Each task is a `CronTaskState` (`{ name, intervalMinutes, schedule, timezone, paused, lastRunAt, nextRunAt }`); each history entry is a `CronHistoryEntry` (`{ taskName, startedAt, completedAt, success, durationMs, error? }`).
