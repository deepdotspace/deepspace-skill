...
| `src/pages/` | File routes (generouted), tiered by route group: **top-level `*.tsx` = static** (no providers — landing/marketing/docs); **`(app)/*.tsx` = dynamic** (providers mount in `(app)/_layout.tsx`; `useAuth`/`useQuery` work only here); **`(app)/(protected)/*.tsx` = dynamic + sign-in gate**. `_app.tsx` is the root shell (ToastProvider only) — **extend, don't replace.** → `references/auth.md` |
...
- **Data/auth hooks only work under `(app)/`.** The DeepSpace providers mount in `src/pages/(app)/_layout.tsx`, not `_app.tsx` — a top-level page calling `useAuth`/`useQuery` crashes at render. The top level of `src/pages/` is reserved for static pages (no auth fetch, no WebSocket — the shipped `index.tsx` landing lives there); `(app)/(protected)/` adds the sign-in gate on top. `npx deepspace add` places feature pages under `(app)/` (or `(app)/(protected)/` for protected ones) automatically. Moving a page across the boundary changes its directory depth — fix up its `../` relative imports.
